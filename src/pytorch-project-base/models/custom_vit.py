...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        # input: (B, N, P, D)
        # B: batch size, N: number of frequency, P: number of patches, D: dimension
        input = x
        count = 0

        for attn, ff in self.layers:
            weight_for_select_freq = self.softmax(self.weights_for_select_freq[count])
            weight = self.sigmoid(self.weights[count])
            insert = (weight_for_select_freq[:,None, None] * input).sum(dim=1)
